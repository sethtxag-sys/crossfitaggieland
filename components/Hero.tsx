...
export default function Hero() {
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center text-center bg-charcoal overflow-hidden">
      ...
      <div className="relative z-[2] pt-24 sm:pt-20 pb-6 sm:pb-10 max-w-[900px] px-6">
        <h1 className="font-display text-[clamp(2.8rem,9vw,7.5rem)] leading-[0.95] uppercase text-white mb-3 sm:mb-6 tracking-wide">
          The Hour That{' '}<br />Changes Everything.
        </h1>
        <p className="text-sm sm:text-lg text-white/70 max-w-[520px] mx-auto mb-4 sm:mb-5 leading-relaxed font-light">
          Elite coaching and a real community in College Station. Your first week is free. No commitment. No strings.
        </p>

        <div className="font-display text-base sm:text-2xl text-white tracking-[3px] sm:tracking-[4px] uppercase mb-3 sm:mb-4">
        ...
        <a
          ...
          className="inline-flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 px-4 sm:px-6 py-2 sm:py-3 bg-white/[0.07] backdrop-blur-sm border border-white/15 rounded-full hover:bg-white/15 transition-all group"
        >
          {/* Google "G" */}
        ...
        {/* CTA buttons — primary is bold, secondary is understated */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
          ...
          <a
            href="#start"
            className="font-display text-sm sm:text-base tracking-widest uppercase text-white/70 hover:text-white transition-colors py-2 sm:py-3"
          >
            See How It Works →
          </a>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 sm:flex sm:justify-center sm:gap-12 mt-6 sm:mt-12">
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className="font-display text-2xl sm:text-3xl text-white tracking-wider">{stat.number}</div>
              <div className="text-[0.6rem] sm:text-[0.7rem] text-white/60 tracking-[1px] sm:tracking-[2px] uppercase mt-0.5 sm:mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
